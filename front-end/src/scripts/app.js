    ...
    ymaps.ready(function () {
        ...
        function listBindEvt() {
            $('#list .item').on('click', function () {
                ...
                if (inArr(spans, value) === -1) {
                    ...
                    $(txt).val('');
                    if (newjson[value].lt) {
                        //myMap.openBalloon(new YMaps.GeoPoint(newjson[value].lt, newjson[value].ln), "Москва");

                        myGeoObject = new ymaps.GeoObject({
                            geometry: {
                                type: "Point", // тип геометрии - точка
                                coordinates: [newjson[value].ln, newjson[value].lt] // координаты точки
                            }
                        });
        ...
        function search(term, suggest) {
            ...
            for (var k = 0; k < choices.length; k++)
                if (~choices[k].toLowerCase().indexOf(term))
        ...
        {
            $.getJSON($(src).attr('href'), function (data) {
                json = data;
